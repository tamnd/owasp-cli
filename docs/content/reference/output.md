...
describes what users get. Pick a format with `-o`, or let owasp choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
owasp <command> -o table   # aligned columns for reading
owasp <command> -o jsonl   # one JSON object per line, for piping
owasp <command> -o json    # a single JSON array
owasp <command> -o csv     # spreadsheet friendly
owasp <command> -o tsv     # tab-separated
owasp <command> -o url     # just the URL column
owasp <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
owasp <command> --fields id,title,url
```
...
```bash
owasp <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
owasp <command>            # a table, because this is a terminal
owasp <command> | wc -l    # JSONL, because this is a pipe
```
